...
public class SupertagFactoredModel extends Model {
	...
	@Override
	public void buildAgenda(final PriorityQueue<AgendaItem> agenda,
			final List<InputWord> words) {
		for (int i = 0; i < words.size(); i++) {
			final InputWord word = words.get(i);
			for (final ScoredCategory cat : tagsForWords.get(i)) {
				agenda.add(new AgendaItem(new SyntaxTreeNodeLeaf(word.word,
						word.pos, word.ner, cat.getCategory(), i), cat
						.getScore(), getOutsideUpperBound(i, i + 1), i, 1,
						false));
			}
		}
	}
	...
	@Override
	public AgendaItem combineNodes(final AgendaItem leftChild,
			final AgendaItem rightChild, SyntaxTreeNode node) {
		final int length = leftChild.spanLength + rightChild.spanLength;

		final List<UnlabelledDependency> resolvedUnlabelledDependencies = node
				.getResolvedUnlabelledDependencies();
		int i = 0;
		for (final UnlabelledDependency dep : resolvedUnlabelledDependencies) {
			node = new SyntaxTreeNodeLabelling(node,
					dep.setLabel(SRLFrame.NONE),
					resolvedUnlabelledDependencies.subList(i + 1,
							resolvedUnlabelledDependencies.size()));

			i++;

		}

		return new AgendaItem(node, leftChild.getInsideScore()
				+ rightChild.getInsideScore(), getOutsideUpperBound(
						leftChild.startOfSpan, leftChild.startOfSpan + length),
						leftChild.startOfSpan, length, false);
	}

	@Override
	public AgendaItem unary(final AgendaItem child,
			final SyntaxTreeNode result, final UnaryRule rule) {
		return new AgendaItem(result, child.getInsideScore(),
				child.outsideScoreUpperbound, child.startOfSpan,
	...
	public static class SupertagFactoredModelFactory extends ModelFactory {
		private final TaggerEmbeddings tagger;

		public SupertagFactoredModelFactory(final TaggerEmbeddings tagger) {
			super();
			this.tagger = tagger;
		}
