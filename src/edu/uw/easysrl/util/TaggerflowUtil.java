...
public class TaggerflowUtil {
	...
	private static TaggingInput toTaggingInput(Stream<Stream<String>> sentences) {
		return TaggingInput.newBuilder().addAllSentence(() -> sentences.map(TaggerflowUtil::toSentence).iterator())
				.build();
	}

	private static InputToParser toParserInput(TaggedSentence taggedSentence, List<Category> categories) {
		final List<List<Tagger.ScoredCategory>> tagDist = TaggerflowLSTM.getScoredCategories(taggedSentence,
				categories);
		final List<InputWord> words = taggedSentence.getTokenList().stream().map(TaggerflowProtos.TaggedToken::getWord)
				.map(word -> new InputWord(word, null, null)).collect(Collectors.toList());
	...
	public static Stream<List<Util.Scored<SyntaxTreeNode>>> parseBatched(Stream<Stream<String>> sentences,
			List<Category> categories, Taggerflow taggerflow, Parser parser) {
		return taggerflow.predict(toTaggingInput(sentences)).getSentenceList().stream()
				.map(ts -> toParserInput(ts, categories)).map(parser::doParsing);
	}
}
